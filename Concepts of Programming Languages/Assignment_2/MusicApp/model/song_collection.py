...
collection_song = Table('collection_song', Base.metadata,
                      Column('collection_id', Integer,
                             ForeignKey('song_collection.id')),
...
class SongCollection(Base):
    ...
    desc = Column(String)
    type = Column(String)
    songs = relationship('Song', secondary=collection_song)

    __mapper_args__ = {
        'polymorphic_identity': 'song_collection',
        'polymorphic_on': type
    }
